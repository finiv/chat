<div
    ...
    x-init="
        height= roomElement.scrollHeight;
        $nextTick(() => roomElement.scrollTop= height);
        Echo.private('users.{{Auth()->User()->id}}')
        .notification((notification)=>{
            if(notification['type']== 'App\\Notifications\\MessageRead' && notification['room_id']== {{$this->selectedRoom->id}})
            {
 ...
 $nextTick(()=>
 roomElement.scrollTop = roomElement.scrollHeight
 );
 "
    ...
    <div class="border-b flex flex-col overflow-y-scroll grow h-full">
        <header class="w-full sticky inset-x-0 flex pb-[5px] pt-[5px] top-0 z-10 bg-white border-b ">
            <div class="flex w-full items-center px-2 lg:px-4 gap-2 md:gap-5">
                ...
                <h6 class="font-bold truncate"> {{ $selectedRoom->getReceiver()->email }} </h6>
            </div>
        </header>
        <main
            @scroll="
      ...
      if(scropTop <= 0){
        ...
        window.livewire.emit('loadMore');
         ...
         height=newHeight;
     "
            id="room" class="flex flex-col gap-3 p-2.5 overflow-y-auto  flex-grow overscroll-contain overflow-x-hidden w-full my-auto">
