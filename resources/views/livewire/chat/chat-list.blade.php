<div
    x-data="{type:'all',roomId:@entangle('roomId')}"
    x-init="

   setTimeout(()=>{
    roomElement = document.getElementById('room-'+roomId);
    if(roomElement){
        ...
        <ul class="p-2 grid w-full spacey-y-2">
            @if ($rooms->count())
                @foreach ($rooms as $key=> $room)
                    <li
                        id="room-{{ $room->id }}" wire:key="{{ $room->id }}"
                        class="py-3 hover:bg-gray-50 rounded-2xl dark:hover:bg-gray-700/70 transition-colors duration-150 flex gap-4 relative w-full cursor-pointer px-2 {{ $room->id == $selectedRoom?->id ? 'bg-gray-100/70' : '' }}">
                        <a href="#" class="shrink-0">
                            <x-avatar src="https://source.unsplash.com/500x500?face-{{$key}}"/>
                        </a>
                        <aside class="grid grid-cols-12 w-full">
                            <a href="{{route('chat',$room->id)}}" class="col-span-11 border-b pb-2 border-gray-200 relative overflow-hidden truncate leading-5 w-full flex-nowrap p-1">
                                <div class="flex justify-between w-full items-center">
                                    <h6 class="truncate font-medium tracking-wider text-gray-900">
                                        {{ htmlspecialchars($room->getReceiver()->name) }}
                                    </h6>
                                    <small class="text-gray-700">{{ htmlspecialchars($room->messages?->last()?->created_at?->shortAbsoluteDiffForHumans()) }} </small>
                                </div>
                                <div class="flex gap-x-2 items-center">
                                    @if ($room->messages?->last()?->sender_id==auth()->id())
                                        @if ($room->isLastMessageReadByUser())
                                            <span>
                                    ...
                                    <x-slot name="content">
                                        <div class="w-full p-1">
                                            <button
                                                ...
                                                wire:click="deleteByUser('{{ encrypt($room->id) }}')"
